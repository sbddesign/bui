...
// Paths
const tailwindThemePath = path.join(
  __dirname,
  'lib',
  'tailwindcss',
  'tailwind-theme.css'
...
function oklchToHex(oklchValue) {
  try {
    ...
    console.warn(
      `Warning: Could not convert OKLCH value "${oklchValue}" to hex:`,
      error.message
    );
    return null;
  }
...
function convertOklchToHex() {
  ...
  try {
    ...
    console.log(
      `🎨 Color families: ${Object.keys(tokens.tailwind).join(', ')}`
    );
    ...
    console.log(
      `   tailwind.${firstColor}.${firstNumber}: ${tokens.tailwind[firstColor][firstNumber].$value}`
    );
  } catch (error) {
